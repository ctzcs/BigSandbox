...
Shader "Universal Render Pipeline/2D/WorldSpaceNSpritesShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void setup()
            {
                ...
                //transform._m23 = -zValue;
                
                unity_ObjectToWorld = mul(transform, offset_matrix(pivot, scale));
                // 提前计算排序用的z值
                int2 sortingData = _sortingDataBuffer[propertyIndex];
                float transformY = unity_ObjectToWorld._m13;
                float yOffset = _sortingGlobalData.y *(saturate(RemapInternal(transformY,_bound.z, _bound.w, 0, 1)));
                
                float zValue = -sortingData.x * _sortingGlobalData.x +
                            -sortingData.y * _sortingGlobalData.y + yOffset;
                //TODO 这里的transformY是获得的,当只有transformY的时候，可以进行Y排序，所以说明，这里的值就是一个是比例问题，一个是yOffset问题
                //要弄清楚比例，以及yOffset的问题
                unity_ObjectToWorld._m23 = zValue;
